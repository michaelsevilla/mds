...
void Client::trim_cache()
{
  ...
  while (lru.lru_get_size() != last) {
    ...
    ldout(cct, 0) << "MSEVILLA: client_cache_size; trimming cache, trim_cache size " << lru.lru_get_size() << " max " << lru.lru_get_max() << dendl;
    trim_dentry(dn);
  }
  ...
  if (lru.lru_get_size() == 0 && root && root->get_num_ref() == 0 && inode_map.size() == 1) {
    ...
    ldout(cct, 0) << "MSEVILLA: client_cache_size; hosing root " << root << dendl;
    delete root;
    root = 0;
    inode_map.clear();
  }
