...
for nclients in {1..6}; do
    CONFIG="$CONFIGDIR/cluster-$nclients.sh"
    source $CONFIG
    mkdir $NFSOUT
    ...
    for i in {0..2}; do
        ...
        $SCRIPTS/ssh-all.sh MDSs $CONFIG \
            "sudo lttng destroy; \
             ...
             sudo lttng enable-event -u --tracepoint \"mds:req*\" -c 0; \
        ...
        JOB="mpirun --mca btl_tcp_if_include eth1 --host $TARGETS /user/msevilla/programs/mdtest/mdtest -n 10 -F -C -d /mnt/cephfs/shared"
        echo 
        echo "-----------------------------"
        ...
        echo "-----------------------------"
        $SCRIPTS/ssh-all.sh MDSs $CONFIG \
            "sudo /usr/bin/lttng stop; \
             sudo chown -R msevilla:msevilla $OUT/lttng_traces"
